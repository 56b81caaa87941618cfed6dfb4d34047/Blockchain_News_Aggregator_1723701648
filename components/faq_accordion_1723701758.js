Vue.component('faq_accordion_1723701758', {
  template: `
    <div class="bg-gradient-to-r from-purple-600 to-pink-500 min-h-screen py-12">
      <section class="leading-relaxed max-w-screen-xl mx-auto px-4 md:px-8">
        <div class="space-y-3 text-center">
          <h1 class="text-4xl text-white font-semibold pt-10">Frequently Asked Questions</h1>
          <p class="text-pink-100 max-w-lg mx-auto text-lg">Everything you need to know about our blockchain information discovery app</p>
        </div>
        <div class="mt-14 max-w-2xl mx-auto">
          <div v-for="(item, index) in faqs" :key="index" class="space-y-5 mt-5 overflow-hidden border-b border-pink-300 hover:bg-white hover:bg-opacity-10 cursor-pointer rounded-lg backdrop-filter backdrop-blur-lg shadow-lg transition duration-300" @click="toggleFaq(index)">
            <h4 class="cursor-pointer pb-5 flex items-center justify-between text-lg text-white font-medium p-4">
              { item.q }
              <svg v-if="selectedFaq === index" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-pink-300 ml-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4"/></svg>
              <svg v-else xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-pink-300 ml-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
            </h4>
